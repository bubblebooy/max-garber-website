  ...
  render () {
    ...
    return (
      <div className="SubFeature">
        ...
        <img src={this.props.imgSrc[this.state.selected]} className="display"/>
        <h3> {this.props.imgTitles ? this.props.imgTitles[this.state.selected] : null} </h3>
        <p>{this.props.description}</p>
        <div className="thumnails">
          {this.props.imgSrc.map((src, index)=>{
            let className="";
            (this.state.selected === index) ? className= "selected" : className = className
            return(
              <img key={index} src={src} className={className} onClick={(e)=>this.onClickHandler(e,index)}/>
            )
          })}
        </div>
      </div>
